...
def run(chapter_path: Path, build_root: Path | None = None) -> dict:
    ...
    mermaid_results = render_from_file(chapter_path, assets_dir / "mermaid")
    result["mermaid"] = mermaid_results

    # 3. QR — kodlar için QR üret
    qr_dir = build_root / "assets" / "qr"
    qr_dir.mkdir(parents=True, exist_ok=True)
    qr_results: list[dict] = []
    for item in build_result.get("extraction", []):
        if item["status"] == "extracted":
            url = f"https://github.com/bmdersleri/bookMaker/blob/main/build/code/{item['code_id']}/{item['file']}"
    ...
    docx_result = export_docx(chapter_path, docx_path)
